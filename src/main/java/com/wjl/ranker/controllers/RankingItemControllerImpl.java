...
public class RankingItemControllerImpl implements RankingItemController {
    ...
    @Override
    public ResponseEntity<List<RankingItemDTO>> getRankingItemsByCategoryId(Long categoryId) {
        List<RankingItem> response = rankingItemService.getAllByCategoryId(categoryService.getById(categoryId));
        return ResponseEntity.ok(response.stream().map(this::toDTO).toList());
    }
